...
export default function HomePage({ switchToLogin, switchToRegister, switchToUpload, onDocumentClick }) {
  ...
  // Đảm bảo categoryId từ URL được set lại sau khi categories load xong
  useEffect(() => {
    const urlParams = new URLSearchParams(window.location.search);
    const categoryIdFromUrl = urlParams.get('categoryId');
    ...
    if (categoryIdFromUrl && categories.length > 0) {
      ...
      const categoryExists = categories.some(cat => (cat._id || cat.id) === categoryIdFromUrl);
      if (categoryExists && filters.categoryId !== categoryIdFromUrl) {
        setFilters(prev => ({ ...prev, categoryId: categoryIdFromUrl }));
      }
    }
  }, [categories]);
        ...
        <div className="filters-container">
          <select
            ...
            onChange={(e) => {
              ...
              // Update URL khi thay đổi category để giữ sync
              const urlParams = new URLSearchParams(window.location.search);
              if (e.target.value) {
                urlParams.set('categoryId', e.target.value);
              } else {
                urlParams.delete('categoryId');
              }
              const newUrl = urlParams.toString() ? `/home?${urlParams.toString()}` : '/home';
