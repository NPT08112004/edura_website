...
export default function DocumentViewer({ documentId, onBack }) {
  ...
  const checkFavoriteStatus = async () => {
    ...
    try {
      // Kiểm tra xem tài liệu có trong danh sách đã lưu không
      const response = await fetch(`${BASE_URL}/api/mobile/documents/saved`, {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('edura_token')}`
        }
      });
      if (response.ok) {
        const data = await response.json();
        const savedDocIds = (data.items || []).map(item => item._id);
        setIsFavorite(savedDocIds.includes(documentId));
      }
    } catch (error) {
    ...
    try {
      ...
      Swal.fire({
        ...
        text: newFavoriteStatus 
          ? 'Tài liệu đã được thêm vào danh sách đã lưu' 
          : 'Tài liệu đã được gỡ khỏi danh sách đã lưu',
        timer: 1500,
      ...
      Swal.fire({
        icon: 'error',
        title: 'Lỗi',
        text: error.message || 'Không thể lưu/bỏ lưu tài liệu'
      });
    } finally {
        ...
        <header className="scribd-topbar">
          ...
          <div className="topbar-center">
            ...
            <button 
              ...
              style={{ 
                color: isFavorite ? '#2563EB' : 'inherit',
                opacity: favoriteLoading ? 0.6 : 1
              }}
            >
              <Bookmark size={18} fill={isFavorite ? '#2563EB' : 'none'} />
            </button>
